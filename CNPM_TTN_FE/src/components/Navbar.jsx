...
import { Link, useNavigate, NavLink } from "react-router-dom"; // Thêm NavLink vào đây
      ...
      <nav className="font-nunito absolute top-0 left-0 z-50 w-full flex flex-col items-center px-8 lg:px-16 py-5 text-white">
        ...
        <div className="w-full flex items-center justify-between gap-4">
          ...
          <div className="flex-1 flex items-center gap-4">
            ...
            <a
              href="#"
              className="border rounded-full p-3 border-accent-1 text-accent-1 hover:bg-accent-1 hover:text-white hover:scale-105 transition-all duration-300"
            >
              <FaFacebookF size={16} />
            </a>
            <a
          ...
          <div className="flex-1 flex justify-end items-center gap-4">
            ...
            <Link
              to="/orders"
              className="relative w-11 h-11 rounded-full border border-white/20 flex items-center justify-center hover:bg-accent-1 hover:scale-110 transition-all duration-300"
            >
            ...
            {user ? (
              <div className="relative group flex items-center gap-3">
                ...
                <div className="absolute -right-2 top-12 w-44 bg-zinc-900 border border-white/10 rounded-xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 overflow-hidden text-sm text-center">
                  {user.role !== 0 && (
                    <Link
                      to="/admin"
                      className="block px-4 py-3 bg-accent-1/10 text-accent-1 font-bold hover:bg-accent-1 hover:text-white border-b border-white/10 transition-colors"
                    >
                      Quản lý hệ thống
                    </Link>
                  )}
              ...
              <button
                ...
                className="w-11 h-11 rounded-full border border-white/20 flex items-center justify-center hover:bg-accent-1 hover:scale-110 transition-all duration-300"
              >
                <FaUserCircle className="text-white text-xl" />
              </button>
            )}
          ...
          <ul className="flex items-center gap-10 font-bold font-nunito text-xs tracking-[0.2em] uppercase text-white">
            <li>
              ...
              <NavLink to="/" end className={getMenuClass}>
                ...
                <IoIosArrowBack
                  size={12}
                  className="text-accent-1 ml-1 rotate-270"
                />
              </NavLink>
            </li>
            ...
            <li>
              <Link
                to="/policy"
                className="hover:text-accent-1 flex items-center justify-center"
              >
      ...
      >
        ...
        <div className="max-w-4xl text-center text-white flex flex-col items-center pt-30">
          ...
          <p className="text-base md:text-lg text-gray-300 leading-relaxed max-w-2xl mx-auto mb-5">
            REVO Coffe nằm khiêm nhường sâu trong một con ngõ nhỏ 234 Hoàng Quốc
            Việt, thuộc khu Bắc Từ Liêm. Quán có thể khó tìm, nhưng những trải
            nghiệm mà bạn có được rất xứng đáng để bỏ công tìm kiếm.
          </p>
