...
const getFeedbackText = (stars) => {
  ...
  const getProductImage = (item) => {
    ...
    // Quét nhiều kiểu field backend có thể trả về
    const imgUrl =
    ...
    // Nếu backend trả đường dẫn tương đối
...
const currentSteps = [
  ...
  const currentShipperSteps = [
    ...
    {
      id: "Đang trung chuyển",
      label: "Đang trung chuyển",
      icon: <Truck size={18} />,
    },
    {
      id: "Shipper đã nhận",
      label: "Đã lấy đơn hàng",
      icon: <Check size={18} />,
    },
    {
      id: "Đang giao",
...
const refundSteps = isCODOrder
  ...
  const getMappedStatus = (status) => {
    if (status === "Chờ thanh toán" || status === "Đã thanh toán")
      return "Chờ xử lý";
    if (
      ["Shipper đã nhận", "Đang trung chuyển", "Đang giao"].includes(status)
    ) {
      return "Đang giao";
    }
    return status;
  ...
  const handleCancelSubmit = async () => {
    const finalReason =
      cancelReason === "Lý do khác" ? customCancelReason : cancelReason;
    if (!finalReason) {
    ...
    <div className="min-h-screen bg-white py-2 text-[#2D3748] pb-20 selection:bg-amber-100 selection:text-amber-900">
      <div className="container mx-auto px-4 max-w-6xl">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 mb-8">
          <div>
            <h1 className="text-3xl font-bold text-[#1A202C] font-nunito tracking-tight">
          ...
          <div className="px-4 py-1.5 rounded-full text-sm font-bold border flex items-center gap-2 bg-accent-1 text-white border-orange-100">
            ...
            <span>
              {["Đang trung chuyển", "Shipper đã nhận", "Đang giao"].includes(
                order.status
              )
                ? "Đang giao"
                : order.status}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white p-8 rounded-2xl shadow-lg border border-gray-100">
              <h2 className="text-xl font-bold mb-10 text-left">
              ...
              <div className="relative flex justify-between">
                ...
                <span>
                  Dịch vụ <strong>Đảm bảo giao hàng đúng hạn</strong> cam kết
                  đơn hàng sẽ được giao chậm nhất vào ngày{" "}
                  {formatDate(estimatedDate)}. Nhận ngay voucher đền bù nếu đơn
                  hàng đến muộn hơn thời gian trên.
                </span>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden transition-all duration-300">
              ...
              <div
                className={`transition-all duration-300 ease-in-out ${
                  isTrackingOpen
                    ? "max-h-[1200px] p-6 md:p-8 opacity-100"
...
<div className="space-y-8 relative z-10">
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b border-gray-100 bg-gray-50/50">
                      <th className="px-6 py-4 font-bold text-sm text-accent-1">
                        Sản phẩm
                      </th>
                      <th className="px-6 py-4 text-center font-bold text-sm text-accent-1">
                        Số lượng
                      </th>
                      <th className="px-6 py-4 text-right font-bold text-sm text-accent-1">
                        Đơn giá
                      </th>
                      <th className="px-6 py-4 text-right font-bold text-sm text-accent-1">
                        Tổng cộng
                    ...
                    {order.orderDetails &&
                      order.orderDetails.map((item, idx) => (
                        ...
                        >
                            ...
                            <div className="flex items-center gap-4">
                              ...
                              <div className="text-left">
                                ...
                                <div className="text-[11px] text-gray-400 mt-1 space-y-0.5">
                                  ...
                                  <p>
                                    Trọng lượng: {item.weight || "Không rõ"}
                                  </p>
                                </div>
                          ...
                          <td className="px-6 py-6 text-right text-gray-500">
                            {(item.unitPrice || 0).toLocaleString()}₫
                          </td>
                          <td className="px-6 py-6 text-right font-black text-[#1A202C]">
                            {(
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100">
              <h3 className="font-bold mb-6 text-[#1A202C] text-left text-lg">
                Thông tin nhận hàng
              </h3>
              <div className="space-y-5">
                <div className="flex gap-4 text-left">
                  ...
                  <div>
                    <p className="text-sm font-bold text-gray-800">
                      {order.receiverName || "Chưa cập nhật danh tính"}
                    </p>
                    <p className="text-[11px] font-bold tracking-wider text-accent-1 uppercase">
                      Người nhận
                    </p>
                  </div>
                </div>
                <div className="flex gap-4 text-left">
                  ...
                  <div>
                    <p className="text-sm font-bold text-gray-800">
                      {order.receiverPhone || "Chưa có số điện thoại"}
                    </p>
                    <p className="text-[11px] font-bold tracking-wider text-accent-1 uppercase">
                      Số điện thoại
                    </p>
              ...
              <div className="mt-6 p-4 bg-[#F1F5F9] rounded-xl flex gap-3 items-start text-left">
                ...
                <div>
                  <p className="text-xs italic text-gray-500">
                    "{order.shippingNote || "Không có ghi chú thêm cho shipper"}
                    "
                  </p>
                  <p className="text-[11px] font-bold mt-1 text-accent-1 uppercase tracking-wide">
                    Ghi chú
                  </p>
                </div>
              </div>
            ...
            <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100 font-nunito">
              <h3 className="font-bold mb-4 text-left text-lg">
                Chi tiết thanh toán
              </h3>
            ...
            {order.status === "Hoàn thành" && (
              <div className="flex items-center justify-center gap-2 text-center text-sm font-bold text-green-600 bg-green-50 p-4 rounded-2xl border border-green-100 shadow-sm animate-fade-in">
                ...
                <span>
                  Đơn hàng thỏa mãn chính sách bảo hộ: Hỗ trợ trả hàng/hoàn tiền
                  miễn phí trong vòng 7 ngày đầu.
                </span>
              </div>
            )}
          </div>
           ...
           {isReviewModalOpen && selectedProductToReview && (
            ...
            >
              ...
              </div> <h3 className="text-xl font-black text-gray-900">Đánh giá sản phẩm</h3>
            ...
            <div className="flex items-center gap-4 bg-slate-50/80 border border-slate-100 p-4 rounded-2xl mb-6 text-left mt-4">
              <div className="w-14 h-14 bg-white rounded-xl p-1.5 border border-slate-100 shadow-sm shrink-0 flex items-center justify-center">
                ...
                <img
                  src={getProductImage(selectedProductToReview)}
                  className="max-h-full object-contain"
                  alt=""
                  onError={(e) => {
                    e.target.src = defaultImage;
