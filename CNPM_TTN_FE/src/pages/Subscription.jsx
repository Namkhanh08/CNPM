...
export default function Subscription() {
  ...
  const handleSelectProduct = async (product) => {
    ...
    try {
      ...
      setGrindType(detail.grindingOptions?.[0] || null);
    } catch (err) {
  ...
  const grindOptions = productDetail?.grindingOptions || [];
  ...
  const submitSubscription = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        totalPrice: paymentMethod === 'vnpay' ? totalVnPayPrice : totalPeriodPrice
      };

      const { default: axios } = await import('axios');
      await axios.post("https://localhost:7117/api/subscriptions/create", payload, {
        headers: {
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div className="lg:col-span-7 space-y-8">
                  ...
                  <div className="mt-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      {[
                        { id: 'Thứ Ba', label: 'Thứ 3 hàng tuần', desc: 'Xưởng chốt mẻ rang & đóng gói vào Thứ 2' },
                        { id: 'Thứ Năm', label: 'Thứ 5 hàng tuần', desc: 'Xưởng chốt mẻ rang & đóng gói vào Thứ 4' }
                      ].map((day) => (
                        <div
                          key={day.id}
                ...
                <div className="lg:col-span-5 space-y-6">
                  ...
                  <div className="p-5 bg-primary rounded-2xl text-white font-nunito">
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <span className="text-xs line-through text-white/40 font-light">{(selectedProduct?.price * quantity || 250000).toLocaleString('vi-VN')}đ</span>
                    </div>
                    <div className="flex justify-between items-baseline mb-4">
                      <span className="text-sm font-semibold">
                        {paymentMethod === 'vnpay' ? 'Tổng tiền trọn gói:' : 'Tổng tiền tạm tính/Kỳ:'}
                      </span>
                      <span className="text-2xl font-black font-montserrat text-white">
                        {(paymentMethod === 'vnpay' ? totalVnPayPrice : totalPeriodPrice).toLocaleString('vi-VN')}đ
                      </span>
                    </div>
            ...
            <div className="flex flex-col items-center my-6">
              ...
              <div className="w-full bg-pinky-gray/20 rounded-xl p-3 font-nunito text-sm space-y-1.5">
                <div className="flex justify-between">
                  ...
                  <span className="font-bold text-accent-1">{totalVnPayPrice.toLocaleString()} đ</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-primary/60">Nội dung:</span>
                  <span className="font-bold text-primary">REVO SUB {Date.now().toString().slice(-6)}</span>
                </div>
              </div>
            </div>
