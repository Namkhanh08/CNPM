...
export default function OrderDetails() {
    ...
    const shippingDetailAddress = order.ShippingDetailAddress ?? order.shippingDetailAddress;
    const shippingWard = order.ShippingWard ?? order.shippingWard;
    const shippingDistrict = order.ShippingDistrict ?? order.shippingDistrict;
    const shippingProvince = order.ShippingProvince ?? order.shippingProvince;
    const shippingNote = order.ShippingNote ?? order.shippingNote;
    const totalAmount = Number(order.TotalAmount ?? order.totalAmount ?? 0);
    const orderDetailsList = order.OrderDetails ?? order.orderDetails ?? [];
    ...
    const handleCancel = async (targetId) => {
        if (window.confirm("Bạn có chắc chắn muốn hủy đơn hàng này?")) {
            try {
                ...
                setOrder(prev => ({ ...prev, Status: 'Đã hủy', status: 'Đã hủy' }));
    ...
    const handleReorder = () => {
        navigate('/shop');
        console.log("Thực hiện logic mua lại đơn hàng:", orderId);
    };
            ...
            <div className="container mx-auto px-4 max-w-6xl">
                ...
                <div className="flex justify-between items-center mb-16">
                    <button
                        ...
                        className="flex items-center gap-2 text-gray-500 hover:text-accent-1 font-bold transition-colors cursor-pointer"
                    >
                ...
                <div className="flex justify-between items-end mb-8">
                    <div>
                        <h1 className="text-3xl font-bold text-[#1A202C] font-nunito font-black">Chi tiết đơn hàng #{orderId}</h1>
                        <p className="text-gray-400 mt-1 text-left">Ngày đặt: {orderDate ? new Date(orderDate).toLocaleDateString('vi-VN') : 'Đang cập nhật'}</p>
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div className="bg-white p-8 rounded-2xl shadow-lg border border-gray-100">
                            ...
                            {!isCancelled ? (
                                <div className="relative flex justify-between">
                                    <div className="absolute top-5 left-6 right-6 w-[calc(100%-48px)] h-[2px] bg-gray-100 z-0"></div>
                                    <div
                                ...
                                <div className="py-4 text-red-500 font-bold italic text-left">Đơn hàng này đã bị hủy bỏ.</div>
                            )}
                        </div>
                        ...
                        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full text-left ">
                                    <thead>
                                        <tr className="text-gray-400 text-[10px] uppercase tracking-[2px] border-b border-gray-100">
                                                    ...
                                                    <td className="px-6 py-6">
                                                        <div className="flex items-center gap-4">
                                                            ...
                                                            <div className="text-left">
                                                                <p className="font-bold text-[#1A202C]">{productName}</p>
                                                                <p className="text-xs text-gray-400 mt-1">Vị: {flavorNotes || 'Không ghi chú'}</p>
                                                                <p className="text-xs text-gray-400 mt-1">Kiểu xay: {translateGrind(grindingOptionId) || 'Không ghi chú'}</p>
                                                                <p className="text-xs text-gray-400 mt-1">Khối lượng: {weight || 'Không ghi chú'}</p>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td className="px-6 py-6 text-center font-bold">{quantity}</td>
                                                    <td className="px-6 py-6 text-right text-gray-500">{unitPrice.toLocaleString()}₫</td>
                                                    <td className="px-6 py-6 text-right font-black">{(unitPrice * quantity).toLocaleString()}₫</td>
                                                </tr>
                                            );
                                        })}
                                    </tbody>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100">
                            <h3 className="font-bold mb-6 text-[#1A202C] text-left">Thông tin nhận hàng</h3>
                            <div className="space-y-5">
                                <div className="flex gap-4 text-left ">
                                    <User size={18} className="text-accent-1 shrink-0" />
                                    <div>
                                        <p className="text-sm font-bold">{receiverName || "Chưa có tên"}</p>
                                ...
                                <div className="flex gap-4 text-left">
                                    ...
                                    <div>
                                        <p className="text-sm font-bold">{receiverPhone || "Chưa có SĐT"}</p>
                                    ...
                                    <div>
                                        <p className="text-sm font-bold leading-relaxed">
                                            {shippingDetailAddress ?
                                                `${shippingDetailAddress}, ${shippingWard || ''}, ${shippingDistrict || ''}, ${shippingProvince || ''}`
                                                : "Chưa có địa chỉ"}
                                        </p>
                            ...
                            <div className="mt-6 p-4 bg-[#F1F5F9] rounded-xl flex gap-3 items-start text-left">
                                ...
                                <div>
                                    <p className="text-xs italic text-gray-500">"{shippingNote || 'Không có ghi chú'}"</p>
                                    <p className="text-[12px] font-bold mt-1 text-accent-1">Ghi chú</p>
                                </div>
                            </div>
                        </div>
                        ...
                        {/* Tổng cộng */}
                        <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100">
                            <h3 className="font-bold mb-4 text-left">Tổng cộng</h3>
                            <div className="space-y-3 text-sm border-b border-gray-100 pb-4">
                                <div className="flex justify-between text-gray-400">
                                    <span>Tạm tính</span>
                                    <span className="font-bold text-gray-600">{totalAmount.toLocaleString()}₫</span>
                                </div>
                                <div className="flex justify-between text-gray-400">
                            ...
                            <div className="flex justify-between items-center pt-4">
                                ...
                                <span className="text-xl font-black text-[#1A202C]">{(totalAmount + 30000).toLocaleString()}₫</span>
                            </div>
                            ...
                            <div className="mt-8 space-y-3">
                                {showPaymentButton ? (
                                    <button
                                        ...
                                        className="w-full font-bold bg-green-600 text-white py-3.5 rounded-xl uppercase tracking-widest text-xs hover:bg-green-700 shadow-md hover:-translate-y-0.5 active:scale-95 transition-all duration-300 cursor-pointer"
                                    >
                                        Thanh toán
                                    </button>
                                ) : (
                                    <button className="w-full font-bold bg-accent-1 text-white py-3.5 rounded-xl uppercase tracking-widest text-xs hover:bg-primary shadow-md hover:-translate-y-0.5 active:scale-95 transition-all duration-300 cursor-pointer">
                                        Theo dõi đơn hàng
                                    </button>
                                )}
                                <button 
                                    onClick={() => handleEdit(orderId)} 
                                    className="w-full font-bold bg-primary text-white py-3.5 rounded-xl uppercase font-nunito tracking-widest text-xs hover:bg-accent-1 shadow-md hover:-translate-y-0.5 active:scale-95 transition-all duration-300 cursor-pointer"
                                >
                                    Chỉnh sửa
                                </button>
                                ...
                                {isCancelled ? (
                                    ...
                                    <button
                                        ...
                                        className="w-full border border-gray-200 text-primary py-3.5 rounded-xl font-bold uppercase tracking-widest text-xs hover:bg-primary hover:text-white shadow-sm hover:-translate-y-0.5 active:scale-95 transition-all duration-300 cursor-pointer"
                                    >
                                        Mua lại
                                    </button>
                                ) : (
                                    ...
                                    <button
                                        className="w-full border border-red-200 text-red-500 py-3.5 rounded-xl font-bold uppercase tracking-widest text-xs hover:bg-red-500 hover:text-white hover:-translate-y-0.5 active:scale-95 transition-all duration-300 cursor-pointer"
                        ...
                        <div className='text-center text-sm'>Bạn cần hỗ trợ? <span className='text-accent-1 font-bold'><Link className='hover:underline'>Liên hệ chúng tôi.</Link></span></div>
                    </div>
                </div>
            </div>
        </div>
