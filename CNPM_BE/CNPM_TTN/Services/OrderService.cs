...
namespace CNPM_TTN.Services
{
    public class OrderService
    {
        ...
        private readonly CartRepository _cRepo;
        private readonly VoucherService _vSer;
        private readonly ApplicationDbContext _context;


        public OrderService(OrderRepository oRepo, ProductRepository pRepo, CartRepository cRepo, VoucherService vSer, ApplicationDbContext context)
        {
            _oRepo = oRepo;
            _pRepo = pRepo;
            _cRepo = cRepo;
        ...
        public Order CreateOrder(string userId, CreateOrderDto dto)
        {
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        _vSer.ApplyVoucher(order.VoucherCode);
                    }
                ...
                try
                {
                    ...
                    foreach (var detail in details)
                    {
                        ...
                        if (product != null)
                        {
                            ...
                            product.Stock += totalReturn;
                            _pRepo.Update(product);
                        }
                    }

                    // HOÀN TRẢ LƯỢT DÙNG VOUCHER KHI HỦY ĐƠN <--- THÊM MỚI
                    if (!string.IsNullOrEmpty(order.VoucherCode))
                    {
                        _vSer.ReleaseVoucher(order.VoucherCode);
                    }
        ...
        public Order UpdateOrder(int id, UpdateOrderDto dto)
        {
            ...
            if (order.Status != "Đã thanh toán" && order.Status != "Đã xác nhận")
            {
        ...
        public Order ShipperFailOrder(int id)
        {
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    // HOÀN TRẢ LƯỢT DÙNG VOUCHER KHI SHIPPER GIAO THẤT BẠI <--- THÊM MỚI
                    if (!string.IsNullOrEmpty(order.VoucherCode))
                    {
                        _vSer.ReleaseVoucher(order.VoucherCode);
                    }
